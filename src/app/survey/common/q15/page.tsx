...
import { ArrowLeft, ArrowRight, Save, CheckCircle, Shield, Clock, Wallet } from "lucide-react";
...
interface SurveyData {
  items: { [key: string]: SafetyItem };
  otherDescription: string;
}

const createInitialData = (): SurveyData => {
  const items: { [key: string]: SafetyItem } = {};
  SAFETY_ITEMS.forEach((category) => {
    category.items.forEach((item) => {
      items[item.id] = { checked: false, hours: "", amount: "" };
    });
  });
...
const saveSurveyData = async (data: SurveyData) => {
  ...
  localStorage.setItem("survey_q15_draft", JSON.stringify(data));
  return { success: true };
};

const loadSurveyData = (): SurveyData | null => {
  if (typeof window === "undefined") return null;
  const draft = localStorage.getItem("survey_q15_draft");
  return draft ? JSON.parse(draft) : null;
};

export default function SurveyQ15Page() {
  ...
  const handleToggle = (itemId: string) => {
    setData({
      ...
      items: {
        ...
        [itemId]: {
          ...data.items[itemId],
          checked: !data.items[itemId].checked,
        },
      },
    });
  };
  ...
  // 選択数・合計計算
  const selectedCount = Object.values(data.items).filter((item) => item.checked).length;
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm text-slate-500">設問 15 / 16</div>
          </div>
        </div>
      </header>
